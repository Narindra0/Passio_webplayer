      ...
      >
        {albums.slice(0, maxItems).map((album, index) => {
          ...
          return (
            <button
              key={album.id}
              onClick={() => handlePress(album.id)}
              style={{
                flex: `0 0 ${cardWidth}px`,
                borderRadius: 'var(--radius-md)',
                overflow: 'hidden',
                cursor: 'pointer',
                border: `1px solid ${accentColor}22`,   // ~13% opacity
                background: `${accentColor}08`,          // ~3% opacity
                textAlign: 'left',
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.borderColor = `${accentColor}55`; // ~33% opacity
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.borderColor = `${accentColor}22`; // ~13% opacity
              }}
            >
              {/* Cover */}
              <div style={{
